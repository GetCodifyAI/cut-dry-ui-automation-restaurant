...
public class VerifyEditQuantitiesFunctionalityInCatalogPDPTest extends TestBase {
    ...
    String uom1 = "1";
    String uom2 = "2";
    static double itemPriceUOM1 ,itemPriceUOM2,totalPrice1,totalPrice2;
    ...
    public void VerifyEditQuantitiesFunctionalityInCatalogPDP() throws InterruptedException {
        ...
        softAssert.assertEquals(Math.round(totalPrice1 * 100.0) / 100.0,
                ((Math.round(itemPriceUOM1 * 2 * 100.0) / 100.0)+(Math.round(itemPriceUOM2 * 100.0) / 100.0)), "The item has not been selected.");
        Catalog.clickAddToCartMinusIcon(1, uom1);
        totalPrice2 = Customer.getItemPriceOnCheckoutButtonViaPDP();
        softAssert.assertEquals(Math.round(Customer.getItemPriceOnCheckoutButtonViaPDP() * 100.0) / 100.0,
                ((Math.round(itemPriceUOM1 * 100.0) / 100.0)-(Math.round(itemPriceUOM2 * 100.0) / 100.0)), "The item not decrease quantity.");
        Catalog.clickBack();
